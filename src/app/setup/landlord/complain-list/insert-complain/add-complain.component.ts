...
@Component({
  ...
  styleUrl: './add-complain.component.scss'
})
export class AddComplainComponent {
...
//tempend
 ...
 constructor(private fb: FormBuilder, private complainService:ComplainService, private notifyService: NotifyService, private modalService: ModalService, 
  ...
  initializeComplainForm(): void {
    this.complainForm = this.fb.group({
      complainInfoForm: this.fb.group({
        ...
        complainInfoID: new FormControl(''),
        agencyID: new FormControl(this.sessionAgencyID),
        landlordID: new FormControl('L0002'),
        propertyID: new FormControl('', [Validators.required]),
        // stateStatus: new FormControl(''),
      }),
  ...
  propertyToggleDropdown(): void {
    ...
    if((this.complainForm.get('propertyID')?.value==='')&&(!this.isPropertyDropdownOpen))
    {
  ...
  clearPropertySelection(): void {
    this.showPropertyInvalid = true;
    this.complainForm.controls['propertyID'].setValue('');
  ...
  selectProperty(item: { id: string; text: string }): void {
    this.showPropertyInvalid = false;
    this.complainForm.get('propertyID')?.setValue(item.id);
    this.complainForm.get('propertyName')?.setValue(item.text);
    this.loadSegments(item.id);
  ...
  ddlSegments: any[]=[];
  loadSegments(property_id: any):void{
    const segment_id = '';   
    ...
    this.segmentService.getSegmentsExtension<any[]>(segment_id, agency_id,'', property_id)
      .then((response: any) => {
        const resData = response.data || [];
