...
import { TechnicianAssignService } from '../technician-assign.service';
...
technicianAssignFormInit()
{
  this.technicianAssignForm = this.fb.group({
    ...
    complainID: new FormControl('',[Validators.required]),
  });
}
...
selectCategory(event: Event): void {
  ...
  if (selectedItem) {
    this.technicianAssignForm.get('technicianCategoryID')?.setValue(selectedItem.id);
    // this.technicianAssignForm.get('technicianCategoryName')?.setValue(selectedItem.text);
    
    this.technicianAssignForm.get('technicianID')?.setValue('');
    // this.technicianAssignForm.get('technicianName')?.setValue('');

    this.technicianCategoryId = selectedItem.id;
...
selectTechnician(event: Event): void
{
  ...
  if(selectedItem){
      this.technicianAssignForm.get('technicianID')?.setValue(selectedItem.id);
...
if(selectedItem){
  this.technicianAssignForm.get('propertyID')?.setValue(selectedItem.id); 
  // this.technicianAssignForm.get('propertyName')?.setValue(selectedItem.text);
  this.technicianAssignForm.get('tenantID')?.setValue('');
  // this.technicianAssignForm.get('tenantName')?.setValue('');
  this.technicianAssignForm.get('complainID')?.setValue('');
...
selectTenant(event:Event)
{
  ...
  if(selectedItem){
    ...
    this.technicianAssignForm.get('tenantID')?.setValue(selectedItem.id);
...
{
  this.technicianAssignForm.get('complainID')?.setValue(item.id);
...
{
  const technicianCategoryID = this.technicianAssignForm.controls['technicianCategoryID'].value || '';
...
{
  let params = this.technicianAssignForm.value;
  const agencyID = this.sessionAgencyID;
      if (!agencyID) {
        console.error("Error: agencyID is not available.");
        return;
...
onSubmit()
{
  if (this.technicianAssignForm.valid) {
    const formValue = this.technicianAssignForm.value;
